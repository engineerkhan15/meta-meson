...
# SD card image name
SDIMG = "${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.rootfs.amlogic-p212-sd.img"
...
#	parted -s ${SDIMG} unit KiB mkpart primary fat32 ${IMAGE_ROOTFS_ALIGNMENT} $(expr ${BOOT_SPACE_ALIGNED} \+ ${IMAGE_ROOTFS_ALIGNMENT})
	...
	if [ "${MACHINE}" = 'amlogic-p212' ]; then
		mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${MACHINE}.bin ::Image
		mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/meson-gxl-s905x-p212.dtb ::meson-gxl-s905x-p212.dtb
